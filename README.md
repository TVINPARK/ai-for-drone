# Quantum Timer (QT) — Телеметрия для FPV дронов

Система захвата и обработки телеметрии с экрана пилота FPV дрона. Распознаёт данные HUD через OCR, сохраняет в базу данных и генерирует отчёты.
