...
- **ОС:** Windows 10/11 (Linux/macOS с ограничениями)
- **Python:** 3.10 или выше
- **Экран:** Разрешение 1920x1080 (рекомендуется)
- **OCR:** EasyOCR (устанавливается автоматически через pip)
...
Теперь система использует **EasyOCR** — библиотека устанавливается автоматически через `pip`, никаких дополнительных программ не требуется!

```bash
pip install -r requirements.txt
```

**Важно:** При первом запуске EasyOCR автоматически загрузит модели для русского и английского языков (~200 МБ). Это происходит один раз при первой инициализации.

### Что изменилось?
Раньше требовалась ручная установка Tesseract OCR. Теперь:
- ✅ Нет необходимости устанавливать Tesseract отдельно
- ✅ Нет необходимости настраивать PATH
- ✅ Все зависимости устанавливаются через `pip install -r requirements.txt`
- ✅ Поддержка кириллицы из коробки
- ✅ Модели упаковываются в EXE-файл при сборке

### Для разработчиков (опционально)
Если вы хотите использовать Tesseract вместо EasyOCR (например, для сравнения качества):
...
5. **Проверка установки:**
   ...
   tesseract --version
   ```
   Если вы увидели номер версии — установка успешна.
...
- **OCR движок**:
  ...
  - EasyOCR как основной бэкенд для кириллицы и чисел (устанавливается через pip)
  - Tesseract как опциональный fallback (если установлен вручную)
...
- `pandas >= 2.0`
- `plotly >= 5.18`
- `easyocr >= 1.7.0` (основной OCR движок)
- `scipy >= 1.10`
- `Pillow >= 9.0`

**Опционально**: Tesseract OCR для сравнения качества:
...
- pytesseract + Tesseract OCR (опционально)
